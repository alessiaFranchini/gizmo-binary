...
#ifdef COOL_GAMMIE
    double betaGammie = 10.;
    double r2Gammie = P[target].Pos[0]*P[target].Pos[0]+P[target].Pos[1]*P[target].Pos[1]+P[target].Pos[2]*P[target].Pos[2];
    double tcoolGammie = sqrt(All.G*2.*All.SeedBlackHoleMass)/pow(r2Gammie,0.75)/betaGammie/UNIT_TIME_IN_CGS;
    double nHcgs = HYDROGEN_MASSFRAC * rho / PROTONMASS;
//    printf("internal en %g, udot %g",u,u*tcoolGammie);
    double cool = -rho*u*tcoolGammie/pow(nHcgs,2.);     /* hydrogen number dens in cgs units */
    //printf("My cool pre %g %g %g\n",cool,u,tcoolGammie);
#ifndef COOLING_OPERATOR_SPLIT
...
#else   //if COOL_BLACK_BODY skip everything before the following
        double surface_density = evaluate_NH_from_GradRho(SphP[target].Gradients.Density,PPP[target].Hsml,SphP[target].Density,PPP[target].NumNgb,1,target);
        surface_density *= UNIT_SURFDEN_IN_CGS; 
        double kappa_eff = 0.4; //cm**2/g
        double tau_eff = kappa_eff * surface_density;
        double Lambda_Thick_BlackBody = 8./3.*5.67e-5 * (T*T*T*T) * (PROTONMASS/surface_density) / ((tau_eff) * nHcgs);
        double Q=-Lambda_Thick_BlackBody;
        double u = 1./(GAMMA_DEFAULT-1)*BOLTZMANN*T/PROTONMASS/0.5;
	printf("Sigma is %2.12f,tau eff is %2.12f,Q is %2.12g, and T is %2.12f \n",surface_density,tau_eff,Lambda_Thick_BlackBody,T);
	printf("Cooling time is %2.12f \n",(u*rho/nHcgs/nHcgs/Q)/UNIT_TIME_IN_CGS);
#endif
...
#ifndef COOLING_OPERATOR_SPLIT
    /* add the hydro energy change directly: this represents an additional heating/cooling term, to be accounted for in the semi-implicit solution determined here. this is more accurate when tcool << tdynamical */
    if(target >= 0) {Q += SphP[target].DtInternalEnergy / nHcgs;} //pdV work 
#if defined(OUTPUT_COOLRATE_DETAIL)
    if(target >= 0) {SphP[target].HydroHeatingRate = SphP[target].DtInternalEnergy / nHcgs;}
#endif
#endif
