...
CC	= mpiicc		# sets the C-compiler (default, will be set for machine below)
CXX	= mpiic++		# sets the C++-compiler (default, will be set for machine below)
FC	= mpiifort	# sets the fortran compiler (default, will be set for machine below)
...
CC	 =  mpiicc
CXX      =  mpiic++
FC	 =  mpiifort# -fbacktrace -g -ffpe-trap=zero,overflow,invalid -fbounds-check -ffree-line-length-none 

OPTIMIZE = -O3 #-ftrapv#-funroll-loops # speed
...
KROME_OPTS = -g -Wall -ffpe-trap=invalid,overflow,underflow,denormal,zero
## modules to load: 
...
CC       =  mpiicc
CXX	 =  mpiicc
#FC      =  ifort -limf -lifcore -lifport -unroll -xHost -g -fp-model precise #gfortran -ffree-line-length-none
FC	 =  ifort -unroll -xHost -g -fp-model precise #gfortran -ffree-line-length-none
OPTIMIZE = -O3 -funroll-loops #-Wextra #-std=c99
OPTIMIZE += -g #-limf -lifcore -lifport -lirc -lsvml # compiler warnings -lgfortran #
OPTIMIZE += -Wall -L/opt/intel/mkl/lib/intel64/
OPTIMIZE_FORTRAN=-O0 -check all -warn all -fpe0 -u -traceback -warn nounused
GMP_INCL = #
GMP_LIBS = #
FFTW2_HOME = /galileo/home/userexternal/alupi000/fftw-2.1.5/build
#HDF5_HOME  = /opt/hdf5-1.10.2_intel
#GSL_HOME   = /opt/gsl-2.4_intel
GSL_INCL = -I$(GSL_HOME)/include
GSL_LIBS = -L$(GSL_HOME)/lib
FFTW_INCL= -I$(FFTW2_HOME)/include
FFTW_LIBS= -L$(FFTW2_HOME)/lib
