...
EOS_GAMMA=1.0            # Polytropic Index of Gas (for an ideal gas law): if not set and no other (more complex) EOS set, defaults to GAMMA=5/3
#EOS_LOCALLY_ISO          # This is to use a locally isothermal eos, change q in eos/eos.c
EOS_LOCALLY_ISO_FARRIS          # This is to use a locally isothermal eos, change q in eos/eos.c
EOS_GENERAL
...
VISCOSITY_SS                   # This is to pass alphaSS to the code and not eta
...
GRAVITY_ANALYTIC               # specific analytic gravitational force to use instead of or with self-gravity. If set to a numerical value
                                ...
                                # if you want binary potential leave it without numerical value
## ----------------------------------------------------------------------------------------------------
...
#COOL_GAMMIE                     # explicit Gammie cooling
#GAMMIE_COOL                     # implicit Gammie cooling, then you need to activate COOLING as well
...
#ADAPTIVE_TREEFORCE_UPDATE=0.0625      # use the tidal timescale to estimate how often gravity needs to be updated, updating a gas cell's gravity no more often than ADAPTIVE_TREEFORCE_UPDATE * dt_tidal, the factor N_f in Grudic 2020 arxiv:2010.13792 (cite this). Smaller is more accurate, larger is faster, should be tuned for your problem if used.
...
SPLITTING_NEW                   # this is to split/merge particles, defined in merge_split.c
...
MERGESPLIT_HARDCODE_MAX_MASS=(1.0e-7)   # manually set maximum mass for particle merge-split operations (in code units): useful for snapshot restarts and other special circumstances
MERGESPLIT_HARDCODE_MIN_MASS=(0.25e-10)   # manually set minimum mass for particle merge-split operations (in code units): useful for snapshot restarts and other special circumstances
#MERGESPLIT_HARDCODE_MAX_MASS=(1.5e-7)   # manually set maximum mass for particle merge-split operations (in code units): useful for snapshot restarts and other special circumstances
#MERGESPLIT_HARDCODE_MIN_MASS=(0.51e-10)   # manually set minimum mass for particle merge-split operations (in code units): useful for snapshot restarts and other special circumstances
FIND_MIN_BH_STEP
