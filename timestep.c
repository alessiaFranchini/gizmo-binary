...
#ifdef FIND_MIN_BH_STEP
    integertime ti_min_glob;
    for(i = 0,ti_min = TIMEBASE;i<NumPart;i++)
    {
	if(P[i].Type!=5) continue;
	ti_step=get_timestep(i, &aphys, 0);
	if(ti_step < ti_min) {ti_min = ti_step;}
    }
    MPI_Allreduce(&ti_min, &ti_min_glob, 1, MPI_TYPE_TIME, MPI_MIN, MPI_COMM_WORLD);
#endif
...
#endif
#ifdef FIND_MIN_BH_STEP
	if(P[i].Type==5 && ti_step>ti_min_glob) ti_step=ti_min_glob;
#endif
                         ...
                         int flag	/*!< either 0 for normal operation, or finite timestep to get corresponding aphys */ )
{
    ...
    if(P[p].Type == 5)
    {
      ...
      double dt_accr = All.MaxSizeTimestep;
        ...
        double dt_ngbs =  GET_PHYSICAL_TIMESTEP_FROM_TIMEBIN(BPP(p).BH_TimeBinGasNeighbor); /* standard wakeup-type threshold: use this by default here, unless dynamical interaction important (e.g. back-rx term from oscillation of BH c-o-m, which is important for single-sink sims */
