...
struct INPUT_STRUCT_NAME
{
    ...
    MyFloat Luminosity;
#endif   
#ifdef BH_FOLLOW_ACCRETED_MOMENTUM
    MyFloat Accel[3];
#endif 
}
...
#endif
#ifdef BH_FOLLOW_ACCRETED_MOMENTUM
    for(k=0;k<3;k++) {in->Accel[k] = P[i].GravAccel[k];}
#endif
...
static inline void OUTPUTFUNCTION_NAME(struct OUTPUT_STRUCT_NAME *out, int i, int mode, int loop_iteration)
{
    ...
    for(k=0;k<3;k++) {ASSIGN_ADD_PRESET(BlackholeTempInfo[target].accreted_force[k], out->accreted_force[k], mode);}
#endif
#if defined(BH_FOLLOW_ACCRETED_COM)
    ...
    while(startnode >= 0) {
        while(startnode >= 0) {
            ...
            for(n = 0; n < numngb; n++)
            {
                ...
                double Mass_j, Vel_j[3], Acc_j[3], InternalEnergy_j; // velocity kicks to apply to 'j's, accumulate here to apply at bottom of code
                ...
                {   /* accreted quantities to be added [regardless of particle type] */
                    ...
                    for(k=0;k<3;k++) {out.accreted_force[k] += FLT( mcount_for_conserve * (Acc_j[k]));}
#endif
#if defined(BH_FOLLOW_ACCRETED_COM)
                    ...
                    out.accreted_J[0] -= Mass_j*(P[j].Pos[1]*Vel_j[2]-P[j].Pos[2]*Vel_j[1]);
                    out.accreted_J[1] += FLT(mcount_for_conserve * ( dpos[2]*dvel[0] - dpos[0]*dvel[2] ));
                    out.accreted_J[1] -= Mass_j*(P[j].Pos[2]*Vel_j[0]-P[j].Pos[0]*Vel_j[2]);
                    out.accreted_J[2] += FLT(mcount_for_conserve * ( dpos[0]*dvel[1] - dpos[1]*dvel[0] ));
                    out.accreted_J[2] -= Mass_j*(P[j].Pos[0]*Vel_j[1]-P[j].Pos[1]*Vel_j[0]);
...
#endif
                        fprintf(FdBhSwallowDetails,"%g %llu %g %2.7f %2.7f %2.7f %2.7f %2.7f %2.7f %llu %g %2.7f %2.7f %2.7f %2.7f %2.7f %2.7f %2.7f %2.7f %2.7f %2.7f %2.7f\n", All.Time, (unsigned long long)local.ID,local.Mass,local.Pos[0],local.Pos[1],local.Pos[2],local.Vel[0],local.Vel[1],local.Vel[2],  (unsigned long long)P[j].ID, Mass_j, (P[j].Pos[0]-local.Pos[0]),(P[j].Pos[1]-local.Pos[1]),(P[j].Pos[2]-local.Pos[2]), (Vel_j[0]-local.Vel[0]),(Vel_j[1]-local.Vel[1]),(Vel_j[2]-local.Vel[2]), SphP[j].InternalEnergy, tempB[0], tempB[1], tempB[2], SphP[j].Density); fflush(FdBhSwallowDetails);
#endif
                    }  // if(P[j].Type == 0)
