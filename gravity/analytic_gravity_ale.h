...
void FindBinary()
{
    ...
    double BHdata[8] = {-1.};
    double* BHdataall = malloc(8*NTask*sizeof(double)) ;
    MyIDType BHIds[2] = {0};
    MyIDType* BHIdsall = malloc(2*NTask*sizeof(MyIDType));

    All.BHdata[3]=All.BHdata[7]=-1.;

    int i,k,j=0;
    for(i=0;i<NumPart;i++){ 
       if(P[i].Type==5)
       {
		if(P[i].Type==5 && P[i].Ti_current!=All.Ti_Current) drift_particle(i,All.Ti_Current);
		for(k=0;k<3;++k) BHdata[k+j*4]=P[i].Pos[k];
		BHdata[3+j*4]=P[i].Mass;
		BHIds[j++]=P[i].ID;
		if(j>1) break;
       }
    }
//    for(j=0;j<2;j++) {printf("%g %g %g %g \n",BHdata[j*4],BHdata[j*4+1],BHdata[j*4+2],BHdata[j*4+3]);}
    MPI_Allgather(BHdata,8,MPI_DOUBLE,BHdataall,8,MPI_DOUBLE,MPI_COMM_WORLD);
    MPI_Allgather(BHIds,2,MPI_UNSIGNED,BHIdsall,2,MPI_UNSIGNED,MPI_COMM_WORLD);

    i=0;
    for(j=0;j<2*NTask;j++){
        if(BHdataall[3+j*4]>0) {
          for(k=0;k<4;k++)  All.BHdata[k+i*4]=BHdataall[k+j*4]; 
	  All.BHIds[i++]=BHIdsall[j];
          if(i>1) break;
        }
    }
    if(ThisTask==0) for(j=0;j<2;j++) {printf("Found BHs %g %g %g %g %u\n",All.BHdata[j*4],All.BHdata[j*4+1],All.BHdata[j*4+2],All.BHdata[j*4+3],All.BHIds[j]);}
...
void GravAccel_BinaryOrbit()
{
	...
	P[i].GravAccel[0]=P[i].GravAccel[1]=P[i].GravAccel[2]=0.;
        double dp[3]; 
        for(j=0;j<2;j++){
           if(All.BHdata[3+j*4]<0){printf("Fuck, problem with BHs\n"); endrun(857463);}// continue;
           for(k=0;k<3;k++) {dp[k]=P[i].Pos[k]-All.BHdata[k+j*4];}

#if defined(BOX_PERIODIC)
           dp[0] -= boxHalf_X; dp[1] -= boxHalf_Y; dp[2] -= boxHalf_Z;
#endif
           double r2 = dp[0]*dp[0]+dp[1]*dp[1]+dp[2]*dp[2], r = sqrt(r2); 
           if(r2!=0){ for(k=0;k<3;k++) {P[i].GravAccel[k] += -All.BHdata[3+j*4]*dp[k] / (r2*r); }}
        }
	//printf("My accel %d %g %g %g\n",P[i].ID,P[i].GravAccel[0],P[i].GravAccel[1],P[i].GravAccel[2]);
	//double r2=P[i].Pos[0]*P[i].Pos[0]+P[i].Pos[1]*P[i].Pos[1]+P[i].Pos[2]*P[i].Pos[2];
	//if(r2>0) for(k=0;k<3;k++) P[i].GravAccel[k] = -1.*P[i].Pos[k]/pow(r2,1.5);
    }
#ifdef BINARY_SINK_GAS
    double acc_sink[6]={0.,0.,0.,0.,0.,0.};
    double dp[6],r2[2],r[2];
    for(i=0;i<NumPart;i++)
    {
	if(P[i].Type==5 || P[i].Mass<=0) continue;
        if(P[i].Ti_current!=All.Ti_Current) drift_particle(i,All.Ti_Current);

        for(k=0;k<3;k++) {dp[k]=P[i].Pos[k]-All.BHdata[k]; dp[k+3]=P[i].Pos[k]-All.BHdata[k+4];}
#if defined(BOX_PERIODIC)
        dp[0] -= boxHalf_X; dp[1] -= boxHalf_Y; dp[2] -= boxHalf_Z;
        dp[3] -= boxHalf_X; dp[4] -= boxHalf_Y; dp[5] -= boxHalf_Z;
#endif
        r2[0] = dp[0]*dp[0]+dp[1]*dp[1]+dp[2]*dp[2], r2[1]=dp[3]*dp[3]+dp[4]*dp[4]+dp[5]*dp[5];
	r[0] = sqrt(r2[0]), r[1]=sqrt(r2[1]); 
        for(k=0;k<3;k++) 
	{
	  if(r2[0]!=0) acc_sink[k] += P[i].Mass*dp[k] / (r2[0]*r[0]); 
	  if(r2[1]!=0) acc_sink[k+3] += P[i].Mass*dp[k+3] / (r2[1]*r[1]); 
	}
    }
