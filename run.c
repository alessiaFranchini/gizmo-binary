...
#pragma omp for schedule(dynamic)
    ...
    for(j=0;j<N_active;j++)
    {
        ...
        double r2Gammie = P[i].Pos[0]*P[i].Pos[0]+P[i].Pos[1]*P[i].Pos[1]+P[i].Pos[2]*P[i].Pos[2];
        double tcoolGammie = sqrt(All.G*2.*All.SeedBlackHoleMass)/pow(r2Gammie,0.75)/betaGammie;
	//printf("My r %g tcool %g udot %g %g\n",sqrt(r2Gammie),1./tcoolGammie,SphP[i].DtInternalEnergy,-SphP[i].InternalEnergyPred*tcoolGammie);
        SphP[i].DtInternalEnergy += -SphP[i].InternalEnergyPred*tcoolGammie;
    }
...
void calculate_non_standard_physics(void)
{
    ...
    MPI_Barrier(MPI_COMM_WORLD); CPU_Step[CPU_COOLINGSFR] += measure_time(); // finish time calc for SFR+cooling
     ...
     if(All.HighestActiveTimeBin == All.HighestOccupiedTimeBin){
      int i,k;
      double xcm[3],vcm[3],mtot;
      double xcmt[3],vcmt[3],mall;
      xcm[0]=xcm[1]=xcm[2]=vcm[0]=vcm[1]=vcm[2]=mtot=0.;
      for(i=0;i<NumPart;i++)
      {
       	for(k=0;k<3;k++) {xcm[k]+=P[i].Mass*P[i].Pos[k]; vcm[k]+=P[i].Mass*P[i].Vel[k];}
        mtot+=P[i].Mass;
      }
      MPI_Allreduce(xcm,xcmt,3,MPI_DOUBLE,MPI_SUM,MPI_COMM_WORLD);
      MPI_Allreduce(vcm,vcmt,3,MPI_DOUBLE,MPI_SUM,MPI_COMM_WORLD);
      MPI_Allreduce(&mtot,&mall,1,MPI_DOUBLE,MPI_SUM,MPI_COMM_WORLD);
      if(ThisTask==0) printf("System recentering\n");
      for(k=0;k<3;k++) {xcmt[k]/=mall; vcmt[k]/=mall;}
      for(i=0;i<NumPart;i++)
      {
        for(k=0;k<3;k++) {P[i].Pos[k]-=xcmt[k]; P[i].Vel[k]-=vcmt[k]; if(P[i].Type==0) SphP[i].VelPred[k]-=vcmt[k];}
      }
     }
#endif
}
