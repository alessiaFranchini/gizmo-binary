...
{
    double ref_factor=1.0, ref_factor_min = 1./32.;
/*!
 ...
 */
#ifdef SPLITTING_NEW
    double Rref = 4.0, rad = sqrt(P[i].Pos[0]*P[i].Pos[0]+P[i].Pos[1]*P[i].Pos[1]+P[i].Pos[2]*P[i].Pos[2]), delta = 1./(ref_factor_min*16.);
    double A = -(1./delta-16.)*16./169., B = 6.*(1./delta-16.)/169., C = 1-A-B, radmin = 1.5;
//    if(rad>=Rref) {ref_factor = 1; } else {ref_factor = ref_factor_min*(1-rad/Rref)+pow(rad/Rref,2);}
//    if(rad>=Rref) {ref_factor = 1; } else {ref_factor = DMAX(1.3333*(ref_factor_min-1)*(1-rad/Rref)+1,ref_factor_min);}
    if(rad<radmin) ref_factor=ref_factor_min; else if(rad<=Rref) { ref_factor = A*(rad/Rref)*(rad/Rref)+B*(rad/Rref)+C;   }
#endif
/*#ifdef SPLITTING_PLANET
    int k, j=0;
    double radloc = 0.;
    double locpos[3] = {0};
    
    if(All.BHdata[3] < All.BHdata[8]){ for(k=0;k<3;k++) {locpos[k]=All.BHdata[k];}}
    else { for(k=0;k<3;k++) locpos[k]=All.BHdata[k+5];}
    
    //printf("M1 is %g, M2 is %g \n",All.BHdata[3],All.BHdata[8]);
    //printf("Local Pos is %g %g \n",All.BHdata[0],All.BHdata[5]);
    //radloc = sqrt(locpos[0]*locpos[0]+locpos[1]*locpos[1]+locpos[2]*locpos[2]);    

    if(P[i].Type==0){
      radloc = sqrt((P[i].Pos[0]-locpos[0])*(P[i].Pos[0]-locpos[0])+(P[i].Pos[1]-locpos[1])*(P[i].Pos[1]-locpos[1])+(P[i].Pos[2]-locpos[2])*(P[i].Pos[2]-locpos[2])) ;
    } else { radloc=10.;}
    if(radloc<=0.5) { ref_factor=ref_factor_min/4.;}// printf("radloc is %g \n",radloc);}
...
#ifdef REBUILD_TREE_IN_REARRANGE
