...
#ifdef EOS_LOCALLY_ISO
    double cs0 = 0.26321480259, qiso = 0.5, riso = sqrt(P[i].Pos[0]*P[i].Pos[0]+P[i].Pos[1]*P[i].Pos[1]+P[i].Pos[2]*P[i].Pos[2]);
    soundspeed = cs0*pow(riso,-qiso);
    press = pow(soundspeed,2)*SphP[i].Density; 

#endif    
    
#ifdef EOS_LOCALLY_ISO_FARRIS
    double hr0 = EOS_LOCALLY_ISO_FARRIS, qiso = 0.5, riso = 0;
    double dp[3] = {0}, factor = 0;
    double R_ref = 2.;
    int k;
    if(All.BHdata[3]+All.BHdata[8]>0){
      for(k=0;k<3;k++){ dp[k] = P[i].Pos[k]-All.BHdata[k]; riso+=dp[k]*dp[k];}
      riso = sqrt(riso);
      factor = All.BHdata[3]/riso;
      riso = 0;
      for(k=0;k<3;k++){ dp[k] = P[i].Pos[k]-All.BHdata[k+5]; riso+=dp[k]*dp[k];}
      riso = sqrt(riso);
      factor += All.BHdata[8]/riso;
      double separ = 0;
      for(k=0;k<3;k++) separ += (All.BHdata[k]-All.BHdata[k+5])*(All.BHdata[k]-All.BHdata[k+5]);
      separ = sqrt(separ);
      //soundspeed = hr0*pow(separ/(All.BHdata[3]+All.BHdata[8]),qiso)*pow(factor,qiso);
      soundspeed = hr0*pow(R_ref/(All.BHdata[3]+All.BHdata[8]),qiso-0.5)*pow(factor,qiso);
      //printf("cs norm is %g while cs is %g \n",cs0*pow(separ/(All.BHdata[3]+All.BHdata[8]),qiso),soundspeed);
      press = pow(soundspeed,2)*SphP[i].Density; 
    }
    else {
      double riso = sqrt(P[i].Pos[0]*P[i].Pos[0]+P[i].Pos[1]*P[i].Pos[1]+P[i].Pos[2]*P[i].Pos[2]);
      soundspeed = cs0*pow(riso,-qiso);
      press = pow(soundspeed,2)*SphP[i].Density; 
      if(All.TimeStep>0) printf("warning : %g %g \n",All.BHdata[3],All.BHdata[8]);
    }

//    if(All.TimeStep>0) printf("%g %g %g %g %g \n",All.BHdata[3],All.BHdata[7],factor,separ,soundspeed);
#endif    
